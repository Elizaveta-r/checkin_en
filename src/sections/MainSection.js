...
export default function MainSection() {
  return (
    <section
      ...
      className="relative z-1 flex w-full flex-col items-center gap-6"
    >
      <ShadowFlare />
      <div className="pt-7 px-3 xl:px-23 flex flex-col justify-center items-center gap-6">
        <div className="flex items-center gap-2">
          ...
          <p className="font-normal sm:text-xl text-xs">
            Keep a <strong>finger on the pulse</strong> of your business.{" "}
            <strong>Always</strong>.
          </p>
        </div>
        <h1
          id="hero-title"
          className="lg:text-6xl md:text-5xl sm:text-3xl text-2xl font-semibold leading-[125%] text-center wrap-normal"
        >
        ...
        <p className="text-foreground md:text-xl text-sm text-center font-normal leading-[170%]">
          AI verifies employee photos, monitors task completion, and sends smart
          reminders. You only see what truly requires your attention.
        </p>
      </div>
      <div className="flex md:flex-row flex-col md:px-0 px-2 md:gap-6 gap-2 items-center md:w-auto w-full">
        <Button
          ...
          aria-label="Try CheckIn24 — register your workspace"
        />
  ...
  return (
    ...
    >
      <Image
        ...
        loading="eager"
        src={shadowFlare}
        alt="CheckIn24 landing background"
        aria-hidden="true"
      />
    </figure>
  );
